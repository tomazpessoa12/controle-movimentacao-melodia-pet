...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/public/bootstrap') return send(res, 200, { pallets: data.pallets, partials:data.partials||[], receipts: data.receipts, summary: summary(data), productLines: data.settings.productLines || [], defaultProductColor: data.settings.defaultProductColor || '#001834' });
    ...
    if(req.method==='POST'&&/^\/api\/public\/partials\/[^/]+\/return$/.test(url.pathname)){const b=await body(req),id=decodeURIComponent(url.pathname.split('/')[4]),item=(data.partials||[]).find(x=>x.id===id&&x.status==='PENDENTE'),q=Number(b.quantity);if(!item||!Number.isFinite(q)||q<=0||q>Number(item.quantity))throw new Error('Quantidade inválida para devolução.');const pallet=data.pallets.find(x=>x.id===item.sourcePalletId);if(!pallet)throw new Error('Palete de origem não encontrado.');item.quantity-=q;if(item.quantity===0){item.status='DEVOLVIDO';item.returnedAt=now()}pallet.quantity=Number(pallet.quantity)+q;pallet.status='PRONTO';writeData(data);return send(res,200,{partial:item,pallet,returned:q});}
    ...
    if (req.method === 'POST' && url.pathname === '/api/admin/settings') { if (!can(user, ['Administrador'])) return send(res, 403, { error: 'Sem permissão.' }); const b = await body(req); if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(String(b.discordHour||''))) throw new Error('Informe o horário no formato 24h (HH:MM).'); data.settings.discordWebhook = String(b.discordWebhook || '').trim(); data.settings.discordHour = b.discordHour; data.settings.discordDays = (b.discordDays||[]).map(Number).filter(x=>Number.isInteger(x)&&x>=0&&x<=6);data.settings.defaultProductColor=/^#[0-9a-f]{6}$/i.test(String(b.defaultProductColor||''))?String(b.defaultProductColor):'#001834';data.settings.productLines=(b.productLines||[]).map(x=>({name:String(x.name||'').trim(),match:String(x.match||'').trim(),color:/^#[0-9a-f]{6}$/i.test(String(x.color||''))?String(x.color):data.settings.defaultProductColor})).filter(x=>x.name&&x.match); writeData(data); return send(res, 200, { ok: true }); }
